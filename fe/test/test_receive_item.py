...
from fe.access import seller,buyer
from fe import conf
...
class TestSendItem:
    ...
    def test_error_authorization_fail(self):
        ...
        code = s.send_item(self.seller_id, self.order_id)
        assert code == 200
        code=self.buyer.receive_item(self.buyer_id,self.order_id+'_x')
        assert code != 200

    def test_error_receive_fail(self):
        code = self.buyer.add_funds(self.total_price)
        assert code == 200
        code = self.buyer.payment(self.order_id)
        assert code == 200
        code = self.buyer.receive_item(self.buyer_id, self.order_id + '_x')
        assert code != 200
    def test_error_repeatreceive(self):
        code = self.buyer.add_funds(self.total_price)
        assert code == 200
        code = self.buyer.payment(self.order_id)
        assert code == 200
